...
func pgwVPPConfigIPv4() vpp.VPPConfig {
	return vpp.VPPConfig{
		...
		NWIs: []vpp.NWIConfig{
			{Name: "cp", Table: 0},
			{Name: "epc", Table: 100},
			{Name: "sgi", Table: 200, TxTable: 201},
		},
		SetupCommands: []string{
			...
			"ip route add 0.0.0.0/0 table 201 via 10.0.1.3 sgi0",

			"adf create application name IPAPP",
			"adf create version app IPAPP",
			"adf version commit app IPAPP",

			"adf create application name TST proxy",
			"adf create version app TST",
			"adf create rule app TST l7 regex ^https?://theserver[46]-.*",
			"adf version commit app TST",
...
func pgwVPPConfigIPv6() vpp.VPPConfig {
	return vpp.VPPConfig{
		...
		SetupCommands: []string{
			...
			"ip route add ::/0 table 201 via 2001:db8:12::3 sgi0",
...
func tdfVPPConfigIPv4() vpp.VPPConfig {
	return vpp.VPPConfig{
		Namespaces: []vpp.VPPNetworkNamespace{
			...
			{
				// TODO: current variant still not correct, because VPP asks via ARP about
				// fake addresses since they share same subnet with main IPs
				Name:        "ue",
				VPPMac:      MustParseMAC("fa:8a:78:4d:18:01"),
				VPPIP:       MustParseIPNet("10.1.0.2/31"),
				OtherIP:     MustParseIPNet("10.1.0.3/31"),
				VPPLinkName: "access0",
				Table:       100,
			},
			{
				Name:        "sgi",
				VPPMac:      MustParseMAC("fa:8a:78:4d:19:01"),
				VPPIP:       MustParseIPNet("10.0.1.2/31"),
				OtherIP:     MustParseIPNet("10.0.1.3/31"),
				VPPLinkName: "sgi0",
				Table:       200,
			},
		},
		...
		NWIs: []vpp.NWIConfig{
			{Name: "cp", Table: 0},
			{Name: "access", Table: 100, TxTable: 101},
			{Name: "sgi", Table: 200, TxTable: 201},
		},
		SetupCommands: []string{
			"upf specification release 16",
			...
			"ip route add 0.0.0.0/0 table 201 via 10.0.1.3 sgi0",
			"ip route add 0.0.0.0/0 table 101 via 10.1.0.3 access0",
...
func tdfVPPConfigIPv6() vpp.VPPConfig {
	return vpp.VPPConfig{
		...
		SetupCommands: []string{
			...
			"ip route add ::/0 table 101 via 2001:db8:11::3 access0",
			"ip route add ::/0 table 201 via 2001:db8:12::3 sgi0",
