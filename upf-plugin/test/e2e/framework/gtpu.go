...
type GTPURawHook sgw.GTPURawHook
...
type GTPU struct {
	...
	sessions map[*sgw.SimpleSession]struct{}
}
...
func NewGTPU(cfg GTPUConfig) (*GTPU, error) {
	...
	up, err := sgw.NewUserPlaneServer(sgw.UserPlaneConfig{
		...
		GTPURawHook: sgw.GTPURawHook(cfg.GTPURawHook),
	}, 0)
	if err != nil {
	...
	return &GTPU{
		cfg:      cfg,
		up:       up,
		sessions: make(map[*sgw.SimpleSession]struct{}),
	}, nil
}

func (gtpu *GTPU) Start(ctx context.Context) error {
	if len(gtpu.sessions) != 0 {
		return nil
	}
	...
	gtpu.sessions[session] = struct{}{}
	gtpu.cfg.GRXNS.AddCleanup(func() { gtpu.Stop() })

	return nil
}

func (gtpu *GTPU) AddSession(cfg sgw.SimpleSessionConfig) error {
	session := sgw.NewSimpleSession(cfg)
...
func (gtpu *GTPU) Stop() error {
	...
	for session := range gtpu.sessions {
		if err := gtpu.up.UnRegisterSession(session); err != nil {
			gtpu.up.Stop()
...
func (gtpu *GTPU) EndMarkersCh() <-chan uint32 {
	return gtpu.up.EndMarkersCh()
}

func (gtpu *GTPU) ErrorIndicationCh() <-chan message.ErrorIndication {
	return gtpu.up.ErrorIndicationCh()
}
