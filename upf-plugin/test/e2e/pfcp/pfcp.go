...
const (
	...
	// For some reason this are in little endian. TODO: Fix to match spec
	ReportingTriggers0_PERIO = 0x01 // Periodic Reporting
	ReportingTriggers0_VOLTH = 0x02 // Volume Threshold
	ReportingTriggers0_TIMTH = 0x04 // Time Threshold
	ReportingTriggers0_QUHTI = 0x08 // Quota Holding Time
	ReportingTriggers0_START = 0x10 // Start of Traffic
	ReportingTriggers0_STOPT = 0x20 // Stop of Traffic
	ReportingTriggers0_DROTH = 0x40 // Dropped DL Traffic Threshold
	ReportingTriggers0_LIUSA = 0x80 // Linked Usage Reporting
	ReportingTriggers1_VOLQU = 0x01 // Volume Quota
	ReportingTriggers1_TIMQU = 0x02 // Time Quota
	ReportingTriggers1_ENVCL = 0x04 // Envelope Closure
	ReportingTriggers1_MACAR = 0x08 // MAC Addresses Reporting
	ReportingTriggers1_EVETH = 0x10 // Event Threshold
	ReportingTriggers1_EVEQU = 0x20 // Event Quota
	ReportingTriggers1_IPMJL = 0x40 // IP Multicast Join/Leave
	ReportingTriggers1_QUVTI = 0x80 // Quota Validity Time
	// TODO: which bits this should use?
	ReportingTriggers2_REEMR = 0x01 // REport the End Marker Reception
	ReportingTriggers2_UPINT = 0x02 // User Plane Inactivity Timer
...
func pfcpSessionResponse(et sessionEventType) pfcpTransitionFunc {
	return func(pc *PFCPConnection, ev pfcpEvent) error {
		if pc.responseCh != nil {
			pc.log.Trace("captured Session Request Response")
			pc.responseCh <- ev.msg
		}
...
func (pc *PFCPConnection) AcquireResponseCh() <-chan message.Message {
	pc.Lock()
	defer pc.Unlock()
	if pc.responseCh == nil {
		pc.responseCh = make(chan message.Message, 100)
	}
	return pc.responseCh
}
...
func (pc *PFCPConnection) pipelineRequests(
	...
	for cur < len(specs) || inFlight > 0 {
		...
		case <-pc.t.Dying():
			...
			for n := range specs {
				if errs[n] == nil {
					errs[n] = errors.New("PFCP connection closed")
				}
			}
			return results, errs
		case <-tch:
...
func GetMeasurement(m message.Message) (*PFCPMeasurement, error) {
	...
	for _, ur := range urs {
		...
		if trigger, err := ur.UsageReportTrigger(); err != nil {
			return nil, errors.Wrap(err, "can't find UsageReportTrigger IE")
		} else {
			r.Trigger = trigger
		}
