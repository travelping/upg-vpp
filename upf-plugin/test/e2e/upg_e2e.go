...
var _ = ginkgo.Describe("PGW", func() {
	...
	ginkgo.Context("[GTP-U extensions]", func() {
		...
		f.TPDUHook = func(tpdu *gtpumessage.TPDU, fromPGW bool) {
			...
			tpdu.SetSequenceNumber(0)
			tpdu.SetNPDUNumber(0)
			tpdu.SetNextExtensionHeaderType(0x32)
			tpdu.AddExtensionHeaders(gtpumessage.NewExtensionHeader(0x32, []byte{0, 0xff}, 0))
		}

		f.GTPURawHook = func(payload []byte) []byte {
			defer ginkgo.GinkgoRecover()

			n++
			if !corruptTPDU || n%13 != 0 {
	...
	ginkgo.Describe("session measurement", func() {
		...
		ginkgo.Context("[ip rules]", func() {
			...
			ginkgo.BeforeEach(func() {
				...
				f.VPP.Ctl("adf create version app IPAPP")
				f.VPP.Ctl("adf create rule app IPAPP ipfilter permit out ip from %s to assigned", appServerIP)
				f.VPP.Ctl("adf version commit app IPAPP")
		...
		sessionContext("[proxy]", framework.SessionConfig{AppName: framework.HTTPAppName}, func() {
			...
			countFlows := func() int {
				f.Ping("ue", f.ServerIP(), 10, true)
				r, err := f.VPP.Ctl("show upf flows")
				framework.ExpectNoError(err)
				lines := strings.Split(r, "\n")
				n := 0
				for _, l := range lines {
					if strings.HasPrefix(l, "flows for worker") {
						continue
					}
			...
			countProxyFlows := func() int {
				proxySessionStr, err := f.VPP.Ctl("show upf proxy session")
				framework.ExpectNoError(err)
				return strings.Count(proxySessionStr, " id ")
			}

			ginkgo.It("can handle a big number of HTTP connections at once", func() {
				// lower default timeouts to speed up test
				_, err := f.VPP.Ctl("upf flow timeout tcp-established 30")
				framework.ExpectNoError(err)

				verifyConnFlood(f, false)

				// check flows removed
				gomega.Eventually(countFlows, 2*time.Minute, 10*time.Second).Should(gomega.Equal(1))

				// all of the proxy sessions must be cleaned up together with the flows
				gomega.Eventually(countProxyFlows, 15*time.Second, 3*time.Second).Should(gomega.Equal(0))
			...
			ginkgo.It("can handle a big number of HTTP connections at once [netem]", func() {
				...
				gomega.Eventually(countFlows, 150*time.Second, 10*time.Second).Should(gomega.Equal(1))
				gomega.Eventually(countProxyFlows, 15*time.Second, 3*time.Second).Should(gomega.Equal(0))
				deleteSession(f, seid, true)
			})

			ginkgo.It("can survive session creation-deletion loop", func() {
				verifySessionDeletionLoop(f, &seid, framework.SessionConfig{AppName: framework.HTTPAppName})
			})
		})
		...
		ginkgo.Context("[proxy bypass]", func() {
			...
			describeProxyBypass := func(skipIPv6 bool) {
				ginkgo.It("should not proxy traffic when higher precedence PDRs have no app id", func() {
					...
					gomega.Expect(flowStr).NotTo(gomega.ContainSubstring("created:"))
				})
				...
				ginkgo.It("should not prevent ADF from working (no app hit)", func() {
					...
					gomega.Expect(flowStr).To(gomega.ContainSubstring("created:1"))
				})
...
var _ = ginkgo.Describe("ADF Tests", func() {
	f := framework.NewDefaultFramework(framework.UPGModeTDF, framework.UPGIPModeV4)
	ginkgo.Context("basic", func() {
		var seid pfcp.SEID

		ginkgo.BeforeEach(func() {
			sessionCfg := framework.SessionConfig{AppName: framework.HTTPAppName}
			seid = startMeasurementSession(f, &sessionCfg)
		})

		ginkgo.It("verifies that apps are detected", func() {
			cfg := smallVolumeHTTPConfig(&traffic.HTTPConfig{
				UseFakeHostname: true,
			})
			runTrafficGen(f, cfg, &traffic.PreciseTrafficRec{})
			ms := deleteSession(f, seid, true)
		...
		ginkgo.It("verifies that new flows in the session use new version", func() {
			cfg := smallVolumeHTTPConfig(&traffic.HTTPConfig{
				UseFakeHostname: true,
				// make it so flow is not expired fast
				ChunkCount: 10,
			})
			// first flow
			runTrafficGen(f, cfg, &traffic.PreciseTrafficRec{})

			flows1, _ := f.VPP.Ctl("show upf flows")
			gomega.Expect(flows1).To(gomega.ContainSubstring("app=TST"))

			// empty app, should not match anymore
			f.VPP.Ctl("adf create version app TST")
			// no rules
			f.VPP.Ctl("adf version commit app TST")

			// second flow (same session)
			runTrafficGen(f, cfg, &traffic.PreciseTrafficRec{})

			flows2, _ := f.VPP.Ctl("show upf flows")
			deleteSession(f, seid, true)

			// should contain both versions, but order is not guaranteed
			gomega.Expect(flows2).To(gomega.ContainSubstring("app=TST"))
			// second flow should not have app field (no app matched)
			gomega.Expect(strings.Count(flows2, "app=")).To(gomega.Equal(1))
		})

		ginkgo.It("ensures fibs are freed", func() {
			// create app with ip rules
			f.VPP.Ctl("adf create app name TESTAPP")
			// create 3 versions (cleanup starts after 3rd)
			for i := 0; i < 3; i++ {
				f.VPP.Ctl("adf create version app TESTAPP")
				f.VPP.Ctl("adf create rule app TESTAPP ipfilter permit out ip from 192.168.0.0 to assigned")
				f.VPP.Ctl("adf version commit app TESTAPP")
			}

			flows, _ := f.VPP.Ctl("show ip table")
			deleteSession(f, seid, true)

			// ensure active fibs are there but first is deleted
			gomega.Expect(flows).To(gomega.ContainSubstring("table_id:10000004"))
			gomega.Expect(flows).To(gomega.ContainSubstring("table_id:10000002"))
			gomega.Expect(flows).To(gomega.Not(gomega.ContainSubstring("table_id:10000000")))
		})
			...
			for _, ueIP := range ueIPs {
				specs = append(specs, pfcp.SessionOpSpec{
					IEs: framework.SessionConfig{
						UEIP:    ueIP,
						Mode:    f.Mode,
						AppName: framework.HTTPAppName, // disable flowless mode
					}.SessionIEs(),
				})
			}
...
var _ = ginkgo.Describe("UPG Binary API", func() {
	ginkgo.Context("for upf nat pool", func() {
		...
		ginkgo.It("tries to add a nat pool with too long name", func() {
			...
			f.VPP.Ctl("upf nwi name %s vrf 0", strings.Repeat("a", 63))
	...
	ginkgo.Context("for policy based routing", func() {
		...
		ginkgo.It("adds, removes and lists the routing policies", func() {
			...
			stop, err := reqCtx.ReceiveReply(msg)
			gomega.Expect(err).To(gomega.BeNil())
			gomega.Expect(stop).To(gomega.BeFalse())

			gomega.Expect(msg.Identifier).To(gomega.BeEquivalentTo(policy.Identifier))
			gomega.Expect(msg.IP4TableID).To(gomega.BeEquivalentTo(200))
			gomega.Expect(msg.IP6TableID).To(gomega.BeEquivalentTo(math.MaxUint32))

			stop, err = reqCtx.ReceiveReply(msg)
			gomega.Expect(err).To(gomega.BeNil())
			gomega.Expect(stop).To(gomega.BeTrue())
	...
	ginkgo.Context("for NWIs", func() {
		...
		ginkgo.BeforeEach(func() {
			...
			_, err = f.VPP.Ctl("ip table add 42420")
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			_, err = f.VPP.Ctl("ip6 table add 42001")
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			_, err = f.VPP.Ctl("ip6 table add 42421")
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		})
		...
		ginkgo.It("adds, removes and lists the NWIs", func() {
			...
			for {
				...
				gomega.Expect(msg.TxIP4TableID).To(gomega.Equal(uint32(42420)))
				gomega.Expect(msg.TxIP6TableID).To(gomega.Equal(uint32(42421)))
			...
			gomega.Expect(out).To(gomega.ContainSubstring(
				"testing default: ip4-table-id 42000, ip6-table-id 42001, " +
					"tx-ip4-table-id 42420, tx-ip6-table-id 42421, " +
	...
	ginkgo.Context("for UEIP export", func() {
		f := framework.NewDefaultFramework(framework.UPGModeTDF, framework.UPGIPModeV4)
		var testNs *network.NetNS

		ginkgo.BeforeEach(func() {
			ns, err := network.NewNS("lcpexport")
			framework.ExpectNoError(err)

			framework.ExpectNoError(ns.Do(func() error {
				_, err := exec.Command("bash", "-c", `ip link add dummy0 type dummy && ip link set up dummy0`).CombinedOutput()
				return err
			}))
		...
		ginkgo.It("enables/disables the feature", func() {
			path := testNs.Path()
			ifName := "dummy0"
			ifNameLen := uint8(len(ifName))

			req := &upf.UpfUeipExportEnable{
				IsEnable:  true,
				NsPath:    []byte(path),
				NsPathLen: 0,
				TableID:   123,
			...
			reply := &upf.UpfUeipExportEnableReply{}

			gomega.Expect(
				f.VPP.ApiChannel.SendRequest(req).ReceiveReply(reply),
			).To(gomega.Succeed(), "upf_ueip_export_enable")

			out, err := f.VPP.Ctl("show upf ueip export as-ns-route")
			framework.ExpectNoError(err)

			gomega.Expect(out).To(gomega.ContainSubstring("status: enabled"))
			gomega.Expect(out).To(gomega.ContainSubstring("namespace: %s", testNs.Path()))
			gomega.Expect(out).To(gomega.ContainSubstring("Interface: dummy0"))
			gomega.Expect(out).To(gomega.ContainSubstring("Table: 123"))
			...
			gomega.Expect(out).To(gomega.ContainSubstring("status: disabled"))
			...
			listNodes := func() string {
				...
				for _, l := range strings.Split(r, "\n") {
					if strings.HasPrefix(l, "Node: ") {
						nodes = append(nodes, strings.Split(l, " ")[1])
					}
				}
				sort.Strings(nodes)
...
var _ = ginkgo.Describe("Sessions authorization metrics", func() {
	f := framework.NewDefaultFramework(framework.UPGModeTDF, framework.UPGIPModeV4)

	getStatValue :=
		func(f *framework.Framework, stat string) (uint64, error) {
			out, err := f.VPP.DumpStats(stat)
			if err != nil {
				return 0, err
			}
			...
			for _, stat := range out {
				...
				case adapter.SimpleCounterStat:
					for _, thread := range data {
						for _, val := range thread {
							sum += uint64(val)
						}
					}
				default:
					return 0, errors.Errorf("unsupported stat type %T", stat.Data)
				}
			}
			return sum, nil
		}

	verifyStartTrafficURReceived :=
		func(f *framework.Framework) {
			...
			gomega.Expect(rr.UsageReport).To(gomega.HaveLen(1))
			ur := rr.UsageReport[0]

			urt, err := ur.FindByType(ie.UsageReportTrigger)
			framework.ExpectNoError(err)
			_, err = urt.UsageReportTrigger()
			framework.ExpectNoError(err)
			gomega.Expect(urt.HasSTART())

			uri, err := ur.FindByType(ie.UEIPAddress)
			framework.ExpectNoError(err)
			ipf, err := uri.UEIPAddress()
			framework.ExpectNoError(err)
			gomega.Expect(ipf.Flags & 0x2).NotTo(gomega.BeZero()) // V4 bit is set
			framework.ExpectEqual(f.UEIP().To4(), ipf.IPv4Address)
		}

	ginkgo.Context("for unsolicited traffic", func() {
		ginkgo.It("should be increased ", func() {
			sessionCfg := &framework.SessionConfig{
				UEIP:       nil,
				Mode:       f.Mode,
				URRStartEv: true,
			...
			ies = append(ies, sessionCfg.CreateURRs()...)
			ies = append(ies, sessionCfg.CreateFARs(pfcp.ApplyAction_DROP)...)

			f.PFCP.AcquireReportCh()
			_, err := f.PFCP.EstablishSession(f.Context, 0, ies...)
			framework.ExpectNoError(err)

			runTrafficGen(f, &traffic.ICMPPingConfig{
				SourceIP:    f.UEIP(),
				PacketCount: 2,
				Delay:       10 * time.Millisecond,
			}, &traffic.SimpleTrafficRec{})

			verifyStartTrafficURReceived(f)

			for metric, val := range map[string]uint64{
				"/upf/wk/session/unsolicited_packets_dropped": 2,
				"/upf/wk/session/unsolicited_ip_reports":      1,
			} {
				stat, err := getStatValue(f, metric)
				framework.ExpectNoError(err)
				framework.ExpectEqual(stat, val)
			}
		})
	})

	ginkgo.Context("for solicited traffic", func() {
		ginkgo.It("should not be increased ", func() {
			sessionCfg := &framework.SessionConfig{
				UEIP:       f.UEIP(),
				Mode:       f.Mode,
				URRStartEv: true,
			...
			for metric, val := range map[string]uint64{
				"/upf/wk/session/unsolicited_packets_dropped": 0,
				"/upf/wk/session/unsolicited_ip_reports":      0,
			} {
				stat, err := getStatValue(f, metric)
				framework.ExpectNoError(err)
				framework.ExpectEqual(stat, val)
...
var _ = ginkgo.Describe("Accessing SGi interface", func() {
	ginkgo.Context("from UE", func() {
		testFunc := func(f *framework.Framework) {
			var err error

			ueIp1 := f.AddUEIP() // sender
			ueIp2 := f.AddUEIP() // recipient

			ueCfg1 := &framework.SessionConfig{
				UEIP: ueIp1,
				Mode: f.Mode,
			}
			ueCfg2 := &framework.SessionConfig{
				UEIP: ueIp2,
				Mode: f.Mode,
			}
			if f.Mode == framework.UPGModePGW {
				ueCfg1.TEIDPGWs5u = framework.TEIDPGWs5u
				ueCfg1.TEIDSGWs5u = framework.TEIDSGWs5u
				ueCfg1.PGWIP = f.VPPCfg.GetVPPAddress("grx").IP
				ueCfg1.SGWIP = f.VPPCfg.GetNamespaceAddress("grx").IP

				ueCfg2.TEIDPGWs5u = 100
				ueCfg2.TEIDSGWs5u = 200
				ueCfg2.PGWIP = f.VPPCfg.GetVPPAddress("grx").IP
				ueCfg2.SGWIP = f.VPPCfg.GetNamespaceAddress("grx").IP

				f.AddGTPUSession(ueCfg1)
				f.AddGTPUSession(ueCfg2)
			}

			_, err = f.PFCP.EstablishSession(f.Context, 0, ueCfg1.SessionIEs()...)
			framework.ExpectNoError(err)
			_, err = f.PFCP.EstablishSession(f.Context, 0, ueCfg2.SessionIEs()...)
			framework.ExpectNoError(err)

			// first message signifies listener ready, second is result
			channel := make(chan error, 1)

			// Fake router that listens for packets from UE to UE
			go func() {
				defer ginkgo.GinkgoRecover()
				ns := f.VPP.GetNS("sgi")
				ns.Do(func() error {
					handle, err := pcap.OpenLive("sgi0", 9000, true, pcap.BlockForever)
					if err != nil {
						channel <- err
						return nil
					}
					...
					err = handle.SetBPFFilter(fmt.Sprintf("udp and %s dst %s", ipVersion, ueIp2.String()))
					if err != nil {
						channel <- err
						return nil
					}

					// notify that listener is ready
					channel <- nil
					...
					case packet := <-packetSource.Packets():
						...
						if udpLayer == nil {
							channel <- errors.New("no udp layer")
						}
						udp, _ := udpLayer.(*layers.UDP)

						if string(udp.Payload) == "i should be routed outside" {
							channel <- nil
						} else {
							channel <- errors.New("wrong payload")
						}
					case <-time.After(5000 * time.Millisecond):
						channel <- errors.New("Packet did not arrive in time")
					}
					return nil
				})
			}()
			...
			gomega.Expect(<-channel).To(gomega.BeNil())
			...
			gomega.Expect(rec.Stats().IsEmpty()).ToNot(gomega.BeTrue())

			// Send a UDP packet from the first UE to second UE
			ns := f.VPP.GetNS("ue")
			err = ns.Do(func() error {
				localAddr := &net.UDPAddr{IP: ueIp1, Port: 1234}
				remoteAddr := &net.UDPAddr{IP: ueIp2, Port: 9999}
				...
				if _, err = conn.Write([]byte("i should be routed outside")); err != nil {
			...
			// check any errors from listener
			gomega.Expect(<-channel).To(gomega.BeNil())
		}

		ginkgo.Context("[TDF]", func() {
			ginkgo.Context("[IPv6]", func() {
				f := framework.NewDefaultFramework(framework.UPGModeTDF, framework.UPGIPModeV6)
		...
		ginkgo.Context("[PGW]", func() {
			ginkgo.Context("[IPv6]", func() {
				f := framework.NewDefaultFramework(framework.UPGModePGW, framework.UPGIPModeV6)
				ginkgo.It("ping should be ignored", func() {
					testFunc(f)
				})
			})
...
var _ = ginkgo.Describe("[Reporting]", func() {
	...
	ginkgo.Context("Remove URR", func() {
		...
		ginkgo.It("should generate usage report", func() {
			...
			triggers := []byte{0x00, 0x08, 0x00} // TERMR
			gomega.Expect(m.Reports[1][0].Trigger).To(gomega.BeEquivalentTo(triggers))
			gomega.Expect(m.Reports[2][0].Trigger).To(gomega.BeEquivalentTo(triggers))
		})
	})

	ginkgo.Context("Modify URR followed by Delete session", func() {
		f := framework.NewDefaultFramework(framework.UPGModeTDF, framework.UPGIPModeV4)
		ginkgo.It("should generate valid usage report", func() {
			ginkgo.By("Removing URRs from session should trigger Usage Report")
			sessionCfg := &framework.SessionConfig{
				UEIP:        f.UEIP(),
				Mode:        f.Mode,
			...
			ginkgo.By("Updating session by removing FARs/PDRs/URRs for forwarding")
			mbrIEs := append(
				sessionCfg.QueryURRs(),
				sessionCfg.UpdateURRs()..., // also update some state to force deeper change
			)
			m, err := f.PFCP.ModifySession(
				f.VPP.Context(context.Background()), seid,
				mbrIEs...)
			...
			m, err = f.PFCP.DeleteSession(f.VPP.Context(context.Background()), seid)
			framework.ExpectNoError(err, "DeleteSession")

			gomega.Expect(m.Reports).To(gomega.HaveLen(2))
			gomega.Expect(m.Reports[1]).To(gomega.HaveLen(1))
			gomega.Expect(m.Reports[2]).To(gomega.HaveLen(1))
			gomega.Expect(m.Reports[1][0].TotalVolume).NotTo(gomega.BeNil())
			gomega.Expect(*m.Reports[1][0].TotalVolume).To(gomega.BeZero())
	...
	ginkgo.Context("Monitoring time", func() {
		...
		ginkgo.It("should generate split report", func() {
			...
			out, err := f.VPP.Ctl("show upf session")
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			gomega.Expect(string(out)).To(gomega.ContainSubstring("monitoring_time"))
			...
			out, err = f.VPP.Ctl("show upf session")
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			gomega.Expect(string(out)).To(gomega.MatchRegexp(`monitoring_time:[^\n]+? \(disarmed\)`),
		...
		ginkgo.It("should properly handle monitoring time change with a pending split report", func() {
			...
			time.Sleep(sessionCfg.MonitoringTime.Sub(now) + 2*time.Second)
			...
			framework.Logf("Elapsed time since session setup: %v", time.Since(startTime))
...
var _ = ginkgo.Describe("[QoS]", func() {
	ginkgo.Context("Gating", func() {
		f := framework.NewDefaultFramework(framework.UPGModeTDF, framework.UPGIPModeV4)
		ginkgo.It("drops a packet", func() {
			...
			ies := sessionCfg.SessionIEs()
			ies = append(ies, sessionCfg.CreateQER(false, 1000)...)
			...
			gomega.Expect(errors).To(gomega.ContainElement("1 upf-ip4-forward dropped because of QER Gate error"))
		})
	})
	ginkgo.Context("Maximum Bit Rate", func() {
		f := framework.NewDefaultFramework(framework.UPGModeTDF, framework.UPGIPModeV4)
		ginkgo.It("drops packets", func() {
			ginkgo.By("Configuring session")
			sessionCfg := &framework.SessionConfig{
				UEIP:        f.UEIP(),
				Mode:        f.Mode,
			...
			ies := sessionCfg.SessionIEs()
			// creating QER with 1 kbit/s refill
			ies = append(ies, sessionCfg.CreateQER(true, 1)...)

			_, err := f.PFCP.EstablishSession(f.Context, 0, ies...)
			framework.ExpectNoError(err)
			ginkgo.By("Starting some traffic")
			// burst >10000 bytes (default burst size)
			tg, clientNS, serverNS := newTrafficGen(f, &traffic.UDPPingConfig{
				PacketSize:  1000 - 28, // - ip header size - udp header size
				PacketCount: 11,
				Retry:       true,
			...
			gomega.Expect(errors).To(gomega.ContainElement("1 upf-ip4-forward dropped because of QER MBR error"))
		})
	})
})
		...
		ginkgo.It("should not leak memory", func() {
			...
			for i := 0; i < leakTestNumIterations; i++ {
				...
				for j := 0; j < leakTestNumSessions; j++ {
					...
					ies := sessionCfgs[j].SessionIEs()
					ies = append(ies, sessionCfgs[j].CreateQER(false, 1000)...)
				...
				for j := 0; j < leakTestNumSessions; j++ {
					...
					specs[j].IEs = append([]*ie.IE{},
						sessionCfgs[j].UpdateFARs(pfcp.ApplyAction_FORW)...,
					)
				}
				...
				for j := 0; j < leakTestNumSessions; j++ {
					...
					specs[j].SEID = seids[j]
					specs[j].IEs = sessionCfgs[j].UpdateFARs(pfcp.ApplyAction_FORW)
				}
				_, errs = f.PFCP.ModifySessions(f.Context, specs)
				for _, err := range errs {
					framework.ExpectNoError(err)
		...
		ginkgo.It("should not be allowed to conflict on UE IPs and drop the older conflicting session", func() {
			...
			for i := 0; i < 1000; i++ {
				...
				if err == nil {
					...
					verifyPFCPError(err, ie.CauseRuleCreationModificationFailure, newSEID, 1,
						fmt.Sprintf(
							"Conflict on ue_ip_ep4 with session 0 up_seid 0x%016x cp_seid 0x%016x", seid, seid))
					break
				}
			}
	...
	ginkgo.Context("[PGW]", func() {
		...
		ginkgo.It("should not be allowed to conflict on GTPU tunnels", func() {
			...
			verifyPFCPError(err, ie.CauseRuleCreationModificationFailure, 0, 1,
				fmt.Sprintf(
					"Conflict on gtpu_ep with session 0 up_seid 0x%016x cp_seid 0x%016x", seid, seid))
...
var _ = ginkgo.Describe("PGW GTPU", func() {
	describeGTPUTests := func(f *framework.Framework) {
		ginkgo.It("Transmit Error indication", func() {
			ginkgo.By("starting traffic without session")

			tg1, clientNS, serverNS := newTrafficGen(f, &traffic.UDPPingConfig{
				PacketCount: 1,
			}, &traffic.SimpleTrafficRec{})
			framework.ExpectError(tg1.Run(f.Context, clientNS, serverNS))

			var ind gtpumessage.ErrorIndication

			ginkgo.By("waiting for error indication")
			gomega.Eventually(f.GTPUs[0].ErrorIndicationCh(), 1*time.Second).Should(gomega.Receive(&ind))
			gomega.Expect(ind.GTPUPeerAddress.MustGSNAddress()).To(gomega.Equal(f.VPPCfg.GetNamespaceAddress("grx").IP.String()))
			gomega.Expect(ind.TEID()).To(gomega.Equal(uint32(0)))
			gomega.Expect(ind.TEIDDataI.MustTEID()).To(gomega.Equal(uint32(framework.TEIDPGWs5u)))
			gomega.Expect(ind.SequenceNumber).To(gomega.Equal(uint16(0)))
			gomega.Expect(ind.ExtensionHeaders).To(gomega.HaveLen(1))
			gomega.Expect(ind.ExtensionHeaders[0].Type).To(gomega.Equal(uint8(0x40))) // gtpu ext header udp port
			gomega.Expect(ind.ExtensionHeaders[0].Content).To(gomega.HaveLen(2))
			gomega.Expect(binary.BigEndian.Uint16(ind.ExtensionHeaders[0].Content)).To(gomega.Equal(uint16(2152)))

			ginkgo.By("creating session")
			...
			ginkgo.By("not receiving error indications for created session")
			tg2, clientNS, serverNS := newTrafficGen(f, &traffic.UDPPingConfig{
				PacketCount: 5,
				Delay:       time.Millisecond * 20,
			}, &traffic.SimpleTrafficRec{})
			framework.ExpectNoError(tg2.Run(f.Context, clientNS, serverNS))

			gomega.Eventually(f.GTPUs[0].ErrorIndicationCh(), 2*time.Second).ShouldNot(gomega.Receive())

			ginkgo.By("removing session and starting traffic")
			f.PFCP.DeleteSession(f.Context, seid)
			framework.ExpectNoError(err)

			tg3, clientNS, serverNS := newTrafficGen(f, &traffic.UDPPingConfig{
				PacketCount: 1,
			}, &traffic.SimpleTrafficRec{})
			framework.ExpectError(tg3.Run(f.Context, clientNS, serverNS))

			ginkgo.By("receiving error indication")
			gomega.Eventually(f.GTPUs[0].ErrorIndicationCh(), 1*time.Second).Should(gomega.Receive(&ind))
			gomega.Expect(ind.GTPUPeerAddress.MustGSNAddress()).To(gomega.Equal(f.VPPCfg.GetNamespaceAddress("grx").IP.String()))
		})

		ginkgo.It("Not crash by receiving Error indication for deleted session", func() {
			ginkgo.By("Configuring session")
		...
		ginkgo.It("End Marker for changed teid", func() {
			ginkgo.By("creating session")
			...
			ginkgo.By("changing far with end marker")
			cfg.TEIDSGWs5u = framework.ProxyAccessTEID
			cfg.FARSendEndMarker = true
			_, err = f.PFCP.ModifySession(f.Context, seid, cfg.UpdateFARs(pfcp.ApplyAction_FORW)...)
			framework.ExpectNoError(err)

			ginkgo.By("waiting for end marker")
			var markerTEID uint32
			gomega.Eventually(f.GTPUs[0].EndMarkersCh(), 2*time.Second, 50*time.Millisecond).Should(gomega.Receive(&markerTEID))
			gomega.Expect(markerTEID).To(gomega.Equal(uint32(framework.TEIDSGWs5u)))

			// change again without indication
			ginkgo.By("changing far without end marker")
			cfg.TEIDSGWs5u = framework.ProxyCoreTEID
			cfg.FARSendEndMarker = false
			_, err = f.PFCP.ModifySession(f.Context, seid, cfg.UpdateFARs(pfcp.ApplyAction_FORW)...)
			framework.ExpectNoError(err)

			ginkgo.By("waiting for no end marker")
			gomega.Eventually(f.GTPUs[0].EndMarkersCh(), 5*time.Second).ShouldNot(gomega.Receive(&markerTEID))

			ginkgo.By("changing far with end marker again")
			cfg.TEIDSGWs5u = framework.TEIDSGWs5u
			cfg.FARSendEndMarker = true
			_, err = f.PFCP.ModifySession(f.Context, seid, cfg.UpdateFARs(pfcp.ApplyAction_FORW)...)
			framework.ExpectNoError(err)

			ginkgo.By("waiting for end again")
			gomega.Eventually(f.GTPUs[0].EndMarkersCh(), 5*time.Second).Should(gomega.Receive(&markerTEID))
			gomega.Expect(markerTEID).To(gomega.Equal(uint32(framework.ProxyCoreTEID)))
		})
	}
...
var _ = ginkgo.Describe("Error handling", func() {
	...
	ginkgo.It("should be done correctly with unknown Forwarding Policy when creating a session", func() {
		...
		verifyPFCPError(err, ie.CauseInvalidForwardingPolicy, seid, 1,
			"cfar: fp: unknown forwarding policy")
	})
	...
	ginkgo.It("should be done correctly with unknown Forwarding Policy when modifying a session", func() {
		...
		verifyPFCPError(err, ie.CauseInvalidForwardingPolicy, seid, 1,
			"ufar: fp: unknown forwarding policy")
	})
	...
	ginkgo.It("should be done correctly for NAT", func() {
		...
		verifyPFCPError(err, ie.CauseRuleCreationModificationFailure, seid, 1,
			"cfar: fp: nat is not configured")

		// configure some pool to init NAT
		_, err = f.VPP.Ctl("upf nat pool 78.32.0.2 - 78.32.0.25 block_size 512 nwi sgi name testing")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		seid2 := f.PFCP.NewSEID()
		_, err = f.PFCP.EstablishSession(f.Context, seid2, sessionCfg.SessionIEs()...)
		verifyPFCPError(err, ie.CauseRuleCreationModificationFailure, seid2, 1,
			"cfar: fp: unknown nat pool name")
	})
})
...
func describeGTPProxy(title string, ipMode framework.UPGIPMode) {
	ginkgo.Context(title, func() {
		var seid pfcp.SEID
		var cfg *framework.SessionConfig

		startProxySession := func(f *framework.Framework) func() {
			return func() {
		...
		shouldPassTheTraffic := func(f *framework.Framework) func() {
			return func() {
		...
		ginkgo.Context("[GTP-U extensions]", func() {
			...
			f.TPDUHook = func(tpdu *gtpumessage.TPDU, fromPGW bool) {
				...
				if fromPGW {
					gomega.Expect(tpdu.HasExtensionHeader()).To(gomega.BeTrue())
					gomega.Expect(len(tpdu.ExtensionHeaders)).To(gomega.Equal(1))
					eh := tpdu.ExtensionHeaders[0]
					gomega.Expect(eh.Type).To(gomega.BeEquivalentTo(0x32))
					gomega.Expect(eh.Content).To(gomega.Equal(content))
					gomega.Expect(eh.NextType).To(gomega.BeEquivalentTo(gtpumessage.ExtHeaderTypeNoMoreExtensionHeaders))
				} else {
					// add an extension on the way towards the GTP proxy
					tpdu.AddExtensionHeaders(
						gtpumessage.NewExtensionHeader(
							0x32, content,
							gtpumessage.ExtHeaderTypeNoMoreExtensionHeaders),
					)
				}
			}

			ginkgo.BeforeEach(startProxySession(f))
			ginkgo.It("should pass the extensions as-is", shouldPassTheTraffic(f))
		})
		...
		ginkgo.Context("", func() {
			...
			ginkgo.BeforeEach(startProxySession(f))
			ginkgo.It("should pass the traffic", shouldPassTheTraffic(f))

			ginkgo.It("should pass the traffic after rules update", func() {
				// add quers in modification request, but do not change existing rules
				_, err := f.PFCP.ModifySession(f.Context, seid, cfg.CreateQER(true, 1000000)...)
				framework.ExpectNoError(err)

				shouldPassTheTraffic(f)
			})

			ginkgo.Context("multi-bearer support", func() {
				ginkgo.It("traffic should pass after second bearer was added", func() {
					// we want to omit ip addresses in session creation
					ueip1 := f.UEIP()    // this tunnel always created before session
					ueip2 := f.AddUEIP() // additional bearer tunnel
					// same for the other end of proxy
					srvip1 := f.ServerIP()
					srvip2 := f.AddServerIP()

					cfg2 := &framework.SessionConfig{
						PDRIdBase:       50,
						FARIdBase:       50,
						URRIdBase:       50,
						UEIP:            nil, // omit
						Mode:            framework.UPGModeGTPProxy,
						TEIDPGWs5u:      framework.TEIDPGWs5u + 0x10,
						TEIDSGWs5u:      framework.TEIDSGWs5u + 0x10,
						PGWIP:           f.VPPCfg.GetNamespaceAddress("core").IP,
						SGWIP:           f.VPPCfg.GetNamespaceAddress("access").IP,
						ProxyAccessTEID: framework.ProxyAccessTEID + 0x10,
						ProxyCoreTEID:   framework.ProxyCoreTEID + 0x10,
						ProxyAccessIP:   f.VPPCfg.GetVPPAddress("access").IP,
						ProxyCoreIP:     f.VPPCfg.GetVPPAddress("core").IP,
						// Do not include filter to check that it is correctly matches PDR
						// This is test for old control plane rules configuration
						SkipSDFFilter:  true,
						TunnelServerIP: srvip2,
					}

					modify1ies := []*ie.IE{}
					// remove old objects
					modify1ies = append(modify1ies, cfg.DeleteURRs()...)
					modify1ies = append(modify1ies, cfg.DeleteFARs()...)
					modify1ies = append(modify1ies, cfg.DeletePDRs()...)
					// and recreate old objects with newer ids which are higher (to cause reorder of in vec)
					cfg.PDRIdBase += 100
					cfg.FARIdBase += 100
					cfg.URRIdBase += 100
					modify1ies = append(modify1ies, cfg.CreateURRs()...)
					modify1ies = append(modify1ies, cfg.CreateFARs(pfcp.ApplyAction_FORW)...)
					modify1ies = append(modify1ies, cfg.CreatePDRs()...)
					// only now add new bearer
					modify1ies = append(modify1ies, cfg2.CreateURRs()...)
					modify1ies = append(modify1ies, cfg2.CreateFARs(pfcp.ApplyAction_FORW)...)
					modify1ies = append(modify1ies, cfg2.CreatePDRs()...)

					_, err := f.PFCP.ModifySession(f.Context, seid, modify1ies...)
					framework.ExpectNoError(err)

					// hack to create tunnel based on IP, but omit UEIP in PDRs
					cfg2.UEIP = ueip2
					f.AddGTPUSession(cfg2)
					cfg.UEIP = nil
					...
					// now remove additional bearer
					modify2ies := []*ie.IE{}
					modify2ies = append(modify2ies, cfg2.DeleteURRs()...)
					modify2ies = append(modify2ies, cfg2.DeleteFARs()...)
					modify2ies = append(modify2ies, cfg2.DeletePDRs()...)
					// recreate original bearer
					modify2ies = append(modify2ies, cfg.DeleteURRs()...)
					modify2ies = append(modify2ies, cfg.DeleteFARs()...)
					modify2ies = append(modify2ies, cfg.DeletePDRs()...)
					cfg.PDRIdBase += 100
					cfg.FARIdBase += 100
					cfg.URRIdBase += 100
					modify2ies = append(modify2ies, cfg.CreateURRs()...)
					modify2ies = append(modify2ies, cfg.CreateFARs(pfcp.ApplyAction_FORW)...)
					modify2ies = append(modify2ies, cfg.CreatePDRs()...)

					_, err = f.PFCP.ModifySession(f.Context, seid, modify2ies...)
					framework.ExpectNoError(err)
...
func describeNAT(f *framework.Framework) {
	ginkgo.Describe("NAT translations", func() {
		...
		verify := func(sessionCfg framework.SessionConfig) {
			...
			seid := startMeasurementSession(f, &sessionCfg)
			trafficCfg := smallVolumeHTTPConfig(nil)
			trafficCfg.ClientPort = 54321
			trafficRec := &traffic.PreciseTrafficRec{}

			runTrafficGen(f, trafficCfg, trafficRec)
			foundAddr := trafficRec.ClientAddr()
			gomega.Expect(foundAddr).To(gomega.BeEquivalentTo("144.0.0.20:10177"))

			runTrafficGen(f, trafficCfg, trafficRec)
			foundAddr = trafficRec.ClientAddr()
			gomega.Expect(foundAddr).NotTo(gomega.BeEquivalentTo("144.0.0.30:55555"))

			trafficPingCfg := &traffic.ICMPPingConfig{PacketCount: 10}
			trafficPingRec := &traffic.SimpleTrafficRec{}
			runTrafficGen(f, trafficPingCfg, trafficPingRec)

			// recreate all PDRs and FARs
			var modifyIEs []*ie.IE
			modifyIEs = append(modifyIEs, sessionCfg.DeletePDRs()...)
			modifyIEs = append(modifyIEs, sessionCfg.DeleteFARs()...)
			sessionCfg.PDRIdBase += 10
			sessionCfg.FARIdBase += 10
			modifyIEs = append(modifyIEs, sessionCfg.CreatePDRs()...)
			modifyIEs = append(modifyIEs, sessionCfg.CreateFARs(pfcp.ApplyAction_FORW)...)
			_, err := f.PFCP.ModifySession(f.VPP.Context(context.Background()), seid, modifyIEs...)
			framework.ExpectNoError(err, "ModifySession")

			// traffic still should use previous binding
			runTrafficGen(f, trafficCfg, trafficRec)
			foundAddr = trafficRec.ClientAddr()
			gomega.Expect(foundAddr).To(gomega.BeEquivalentTo("144.0.0.20:10177"))
			...
			out, err := f.VPP.Ctl("show upf nat pool")
			framework.ExpectNoError(err)

			// should release nat binding
			gomega.Expect(out).To(gomega.ContainSubstring("blocks usage 0/"))
			gomega.Expect(out).ToNot(gomega.ContainSubstring("blocks usage 1/"))
		}
...
func verifyConnFlood(f *framework.Framework, netem bool) {
	rec := &traffic.SimpleTrafficRec{}
	// First traffic gen trying to reconnect using same source port
	// This can trigger more issues with recreating proxy flow for the same upf flow
	tg1, clientNS, serverNS := newTrafficGen(f, &traffic.HTTPConfig{
		Retry:             true,
		SimultaneousCount: 500, // TODO: 5000 works with bigger chunks but takes up too much memory
		Persist:           true,
		ClientPort:        10000,
	...
	// Second traffic gen just downloads
	tg2, clientNS, serverNS := newTrafficGen(f, &traffic.HTTPConfig{
		Retry:             true,
		SimultaneousCount: 500, // TODO: 5000 works with bigger chunks but takes up too much memory
		Persist:           true,
		ChunkDelay:        -1,  // no delay
	...
	defer cancel()
	tg1Done := tg1.Start(ctx, clientNS, serverNS)
	tg2Done := tg2.Start(ctx, clientNS, serverNS)
	select {
	case <-f.Context.Done():
...
func verifySessionDeletionLoop(f *framework.Framework, seid *pfcp.SEID, cfg framework.SessionConfig) {
	rec := &traffic.SimpleTrafficRec{}
	tg, clientNS, serverNS := newTrafficGen(f, &traffic.HTTPConfig{
		Retry:             true,
		SimultaneousCount: 500, // TODO: 5000 works with bigger chunks but takes up too much memory
		Persist:           true,
		ClientPort:        10000, // Force reuse of port what can trigger more problems with proxy
		ChunkDelay:        -1,    // no delay
		ChunkSize:         100,   // use small chunks to avoid using up too much memory
		ChunkCount:        10000,
	}, rec)
...
// "Session 0 SEIDs UP:0x5581e8d28a7245a5 CP:0x5581e8d28a7245a5 peer=10.0.0.3"
var seidRx = regexp.MustCompile(`UP:0x([0-9A-Fa-f]+)`)
...
func setupNAT(f *framework.Framework) {
	...
	f.VPP.Ctl("upf ueip pool nwi sgi id testing")
}
...
func verifyPFCPError(err error, cause uint8, seid pfcp.SEID, failedRuleID uint32, message string) {
	...
	gomega.Expect(errors.As(err, &serverErr)).To(gomega.BeTrue())
	if seid != 0 {
		framework.ExpectEqual(serverErr.SEID, seid, "SEID")
	}
...
var _ = ginkgo.Describe("UEIP export", func() {
	var testNs *network.NetNS
	...
	showTableFunc := func(t *test) string {
		v := "-6"
		if t.is4 {
			v = "-4"
		}
		var out []byte
		framework.ExpectNoError(testNs.Do(func() error {
			c := exec.Command("ip", v, "route", "show", "table", "123")
	...
	checkShowOutputDisabled := func(f *framework.Framework) {
		out, err := f.VPP.Ctl("show upf ueip export as-ns-route")
		framework.ExpectNoError(err)
	...
	checkShowOutput := func(f *framework.Framework, expectEntries int) {
		out, err := f.VPP.Ctl("show upf ueip export as-ns-route")
		framework.ExpectNoError(err)

		gomega.Expect(out).To(gomega.ContainSubstring("status: enabled"))
		gomega.Expect(out).To(gomega.ContainSubstring("Network namespace: %s", testNs.Path()))
		gomega.Expect(out).To(gomega.ContainSubstring("Interface: dummy0"))
		gomega.Expect(out).To(gomega.ContainSubstring("Table: 123"))
		gomega.Expect(out).To(gomega.ContainSubstring("Routes added: %d\n", expectEntries))
	}

	exportsUEtoTableFunc := func(f *framework.Framework, t test) {
		checkShowOutputDisabled(f)
		_, err := f.VPP.Ctl("upf ueip export as-ns-route enable ns-path %s table 123 if-name dummy0", testNs.Path())
		framework.ExpectNoError(err)
		checkShowOutput(f, 0)
		...
		trafficCfg := smallVolumeHTTPConfig(&traffic.HTTPConfig{UseFakeHostname: true})
		runTrafficGen(f, trafficCfg, &traffic.PreciseTrafficRec{})

		gomega.Expect(showTableFunc(&t)).To(gomega.ContainSubstring(t.routeString))
		checkShowOutput(f, 1)

		sessionCfg.PDRIdBase += 10

		// now we add additional PDRs with same addresses
		_, err = f.PFCP.ModifySession(f.VPP.Context(context.Background()), seid, sessionCfg.CreatePDRs()...)
		framework.ExpectNoError(err)
		runTrafficGen(f, trafficCfg, &traffic.PreciseTrafficRec{})
		checkShowOutput(f, 1)

		// now we remove additional PDRs and validate that route stays
		_, err = f.PFCP.ModifySession(f.VPP.Context(context.Background()), seid, sessionCfg.DeletePDRs()...)
		framework.ExpectNoError(err)
		runTrafficGen(f, trafficCfg, &traffic.PreciseTrafficRec{})
		checkShowOutput(f, 1)
		...
		// now we remove session and validate that routes removed
		deleteSession(f, seid, true)
		checkShowOutput(f, 0)
		gomega.Expect(showTableFunc(&t)).ToNot(gomega.ContainSubstring(t.routeString))
	}

	ginkgo.Context("[IPv4]", func() {
		f := framework.NewDefaultFramework(framework.UPGModeTDF, framework.UPGIPModeV4)
		ginkgo.It("exports UEIP to kernel table", func() {
			exportsUEtoTableFunc(f, test{
				is4:         true,
				routeString: "10.1.0.3 dev dummy0",
			})
		})
	})
	...
	ginkgo.Context("[IPv6]", func() {
		...
		ginkgo.It("exports UEIP to kernel table", func() {
			exportsUEtoTableFunc(f, test{
				is4:         false,
				routeString: "2001:db8:11::/64 dev dummy0",
			})
		})
	})
})
