...
// Use poll-sleep-usec to:
// - reduce system load
// - simulate lags
// - detect more errors when same flow packets end up in the same vector
...
plugins {
  ...
  plugin wireguard_plugin.so { disable }
  plugin ikev2_plugin.so { disable }
  plugin perfmon_plugin.so { disable }
}
...
` + // tcp - increase cleanup time from 0.1s to 1s, so it wont fail due to slow debug output
	`tcp {
  cleanup-time 1000
}
`

// enable vlog for tcp-proxy dumps
// + `vlib {
//   elog-events 100000
// }`
...
func (cfg *VPPStartupConfig) SetFromEnv() {
	...
	if v := os.Getenv("VPP_GDB_SERVER_PORT"); v != "" {
	...
	if v := os.Getenv("VPP_POST_MORTEM_ELOG_LIMIT"); v != "" {
		if lim, err := strconv.Atoi(v); err == nil {
			cfg.PostMortemElogLimit = lim
		}
	}
	cfg.SetDefaults()
}
...
func (cfg *VPPStartupConfig) SetDefaults() {
	...
	if cfg.APIPrefix == "" {
		cfg.APIPrefix = fmt.Sprintf("vpp%d_%d", ginkgo.GinkgoParallelProcess(), atomic.AddInt32(&vppIndex, 1))
	}
	if cfg.GDBServerPort == 0 {
		cfg.GDBServerPort = 7777
	}
	if cfg.PostMortemElogLimit == 0 {
		cfg.PostMortemElogLimit = 10000
	}
}
