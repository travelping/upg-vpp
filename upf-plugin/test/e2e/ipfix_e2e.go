		...
		if ipMode == framework.UPGIPModeV4 && mode == framework.UPGModeTDF {
			ginkgo.Context("[NAT fields]", func() {
				...
				ginkgo.It("includes NAT fields in IPFIX reports", func() {
					...
					trafficCfg.ClientPort = 33333 // used as hash for nat port allocation
					v.verifyIPFIX(ipfixVerifierCfg{
						...
						postNAPTSourceTransportPort: 10181,
					})
					v.verifyIPFIXNatEventRecords()
					v.verifyNAT()
				})
...
func (v *ipfixVerifier) verifyNAT() {
	...
	// 1st IPFIX record for the flow, NAT flow create
	framework.ExpectEqual(v.recs[0]["natEvent"], uint8(4))
	last := len(v.recs) - 1
	// last IPFIX record for the flow NAT flow delete
...
func (v *ipfixVerifier) verifyIPFIXFlowUsageRecords() {
	uplinkVRFName1 := "ipv4-VRF:200" // Default routing
	uplinkVRFName2 := "ipv4-VRF:201" // Strict routing
	if v.f.IPMode == framework.UPGIPModeV6 {
		uplinkVRFName1 = "ipv6-VRF:200"
		uplinkVRFName2 = "ipv6-VRF:201"
	}
	if v.cfg.expectedUplinkVRFName != "" {
		uplinkVRFName1 = v.cfg.expectedUplinkVRFName
		uplinkVRFName2 = v.cfg.expectedUplinkVRFName
	}
	...
	for _, r := range v.recs {
		...
		gomega.Expect(r["VRFname"]).To(gomega.Or(
			gomega.Equal(uplinkVRFName1),
			gomega.Equal(uplinkVRFName2)))
