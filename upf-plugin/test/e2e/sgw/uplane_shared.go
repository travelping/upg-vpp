...
func (ts *tunnelShared) prepareRoute(ip net.IP, link netlink.Link) *netlink.Route {
	...
	if ip.To4() == nil {
		...
		zeroip = net.IPv6zero
		proto = netlink.FAMILY_V6
	} else {
		mask = net.CIDRMask(0, 32)
		zeroip = net.IPv4zero
		proto = netlink.FAMILY_V4
	}
...
func retryNetlinkOperation(operation func() error, maxRetries int) error {
	var lastErr error
	...
	return errors.Wrapf(lastErr, "operation failed after %d retries", maxRetries)
}

func (ts *tunnelShared) addIP(ip net.IP, link netlink.Link) error {
	var addrs []netlink.Addr
	if err := retryNetlinkOperation(func() error {
		var err error
		addrs, err = ts.up.ueHandle.AddrList(link, netlink.FAMILY_ALL)
		return err
	}, 5); err != nil {
	...
	for _, a := range addrs {
		if a.IPNet.IP.Equal(ip) {
			// already added
			return nil // avoid failing here for convenience
		}
	}

	if err := retryNetlinkOperation(func() error {
		return ts.up.ueHandle.AddrAdd(link, ts.prepareIP(ip))
	}, 5); err != nil {
...
func (ts *tunnelShared) addRuleLocal(ip net.IP) error {
	...
	if err := retryNetlinkOperation(func() error {
		return ts.up.ueHandle.RuleAdd(newRule)
	}, 5); err != nil {
