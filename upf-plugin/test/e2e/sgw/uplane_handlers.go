...
func (up *UserPlaneServer) handleMessage(msg message.Message, src *net.UDPAddr) error {
	if msg != nil {
		up.logger.Tracef("Received %q", msg.MessageTypeName())
	}
	...
	case *message.EndMarker:
		up.endMarkersCh <- msg.TEID()
		return nil
	default:
		return errors.Errorf("Missed handler for message %+#v", msg)
	}
}
...
func (up *UserPlaneServer) handleErrorIndication(msg *message.ErrorIndication) error {
	select {
	case up.errorIndicationCh <- *msg:
	default:
		// this is job of tests to consume packets in time
	}
	return nil
}
