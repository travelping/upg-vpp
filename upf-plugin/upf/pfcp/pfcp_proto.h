...
#ifndef UPF_PFCP_PFCP_PROTO_H_
#define UPF_PFCP_PFCP_PROTO_H_
...
typedef struct __attribute__ ((packed))
{
  u64 seid;
  u8 sequence[3];
  // TODO: Get rid of bitfields for deserialization
...
{
  const char *name;
  ssize_t length; // size of IE type data
  u8 size;        // length of sub IEs types list
  union
  {
    struct
    {
...
#define PFCP_SRC_INTF_5G_VN   4
#define PFCP_SRC_INTF_NUM     (PFCP_SRC_INTF_5G_VN + 1)
...
#define PFCP_REPORTING_TRIGGER_REPORT_THE_END_MARKER_RECEPTION BIT (16)
#define PFCP_REPORTING_TRIGGER_USER_PLANE_INACTIVITY_TIMER     BIT (17)
...
enum
{
  ...
  PDI_TRAFFIC_ENDPOINT_ID,
  PDI_LAST = PDI_TRAFFIC_ENDPOINT_ID
};
...
/* PFCP Methods */
/* clang-format off */
#define foreach_pfcp_msg \
/*_ (enum_name,                pfcp_id,  metric_name,                    string name                   ) */ \
  _ (HEARTBEAT_REQUEST,              1,  heartbeat_request,              "Heartbeat Request"              ) \
  _ (HEARTBEAT_RESPONSE,             2,  heartbeat_response,             "Heartbeat Response"             ) \
  _ (PFD_MANAGEMENT_REQUEST,         3,  pfd_management_request,         "PFD Management Request"         ) \
  _ (PFD_MANAGEMENT_RESPONSE,        4,  pfd_management_response,        "PFD Management Response"        ) \
  _ (ASSOCIATION_SETUP_REQUEST,      5,  association_setup_request,      "Association Setup Request"      ) \
  _ (ASSOCIATION_SETUP_RESPONSE,     6,  association_setup_response,     "Association Setup Response"     ) \
  _ (ASSOCIATION_UPDATE_REQUEST,     7,  association_update_request,     "Association Update Request"     ) \
  _ (ASSOCIATION_UPDATE_RESPONSE,    8,  association_update_response,    "Association Update Response"    ) \
  _ (ASSOCIATION_RELEASE_REQUEST,    9,  association_release_request,    "Association Release Request"    ) \
  _ (ASSOCIATION_RELEASE_RESPONSE,   10, association_release_response,   "Association Release Response"   ) \
  _ (VERSION_NOT_SUPPORTED_RESPONSE, 11, version_not_supported_response, "Version Not Supported Response" ) \
  _ (NODE_REPORT_REQUEST,            12, node_report_request,            "Node Report Request"            ) \
  _ (NODE_REPORT_RESPONSE,           13, node_report_response,           "Node Report Response"           ) \
  _ (SESSION_SET_DELETION_REQUEST,   14, session_set_deletion_request,   "Session Set Deletion Request"   ) \
  _ (SESSION_SET_DELETION_RESPONSE,  15, session_set_deletion_response,  "Session Set Deletion Response"  ) \
  _ (SESSION_ESTABLISHMENT_REQUEST,  50, session_establishment_request,  "Session Establishment Request"  ) \
  _ (SESSION_ESTABLISHMENT_RESPONSE, 51, session_establishment_response, "Session Establishment Response" ) \
  _ (SESSION_MODIFICATION_REQUEST,   52, session_modification_request,   "Session Modification Request"   ) \
  _ (SESSION_MODIFICATION_RESPONSE,  53, session_modification_response,  "Session Modification Response"  ) \
  _ (SESSION_DELETION_REQUEST,       54, session_deletion_request,       "Session Deletion Request"       ) \
  _ (SESSION_DELETION_RESPONSE,      55, session_deletion_response,      "Session Deletion Response"      ) \
  _ (SESSION_REPORT_REQUEST,         56, session_report_request,         "Session Report Request"         ) \
  _ (SESSION_REPORT_RESPONSE,        57, session_report_response,        "Session Report Response"        )
/* clang-format on */

typedef enum : u8
{
#define _(N, PFCP_ID, METRIC, STR) PFCP_MSG_##N = PFCP_ID,
  foreach_pfcp_msg
#undef _
    PFCP_N_MSG,
} pfcp_msg_type_t;
...
enum
{
  ...
  SESSION_REPORT_REQUEST_LAST = SESSION_REPORT_REQUEST_TP_ERROR_MESSAGE
};
...
typedef struct
{
  ...
  pfcp_ie_tp_error_message_t tp_error_message;
...
// Decoder relies on this fact to fill grp structure
STATIC_ASSERT (offsetof (pfcp_decoded_msg_t, simple_response.grp) ==
                 offsetof (pfcp_decoded_msg_t, grp),
               "grp union offset don't match");
STATIC_ASSERT (offsetof (pfcp_decoded_msg_t, heartbeat_request.grp) ==
                 offsetof (pfcp_decoded_msg_t, grp),
               "grp union offset don't match");
...
format_function_t format_pfcp_flags;
format_function_t format_pfcp_enum;
format_function_t format_pfcp_dns_labels;
format_function_t format_pfcp_msg_type;
format_function_t format_pfcp_msg_header;
format_function_t format_pfcp_ie_user_plane_ip_resource_information;
format_function_t format_pfcp_ie_redirect_information;
format_function_t format_pfcp_ie_ue_ip_address;
format_function_t format_pfcp_ie_node_id;
format_function_t format_pfcp_ie_f_seid;
format_function_t format_pfcp_ie_smf_set_id;
format_function_t format_pfcp_ie_outer_header_creation;
format_function_t format_pfcp_ie_source_interface;
format_function_t format_pfcp_ie_destination_interface;
uword pfcp_tbcd_len (u8 *in, uword n_bytes);
uword decode_pfcp_tbcd (u8 *in, uword n_bytes, u8 *out, uword n_out);
uword encode_pfcp_tbcd (u8 *in, uword n_bytes, u8 *out, uword n_out);
format_function_t format_pfcp_tbcd;
format_function_t format_pfcp_dmsg;
format_function_t format_pfcp_ie_user_id;
format_function_t format_pfcp_ie_source_interface;

void copy_pfcp_ie_node_id (pfcp_ie_node_id_t *dst, pfcp_ie_node_id_t *src);
