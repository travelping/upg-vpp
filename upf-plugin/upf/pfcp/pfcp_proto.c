...
static const char *_pfcp_msg_names[] = {
#define _(N, PFCP_ID, METRIC, STR) [PFCP_MSG_##N] = STR,
  foreach_pfcp_msg
#undef _
};
...
{
  pfcp_msg_type_t type = va_arg (*args, int);
  return type < ARRAY_LEN (_pfcp_msg_names) && _pfcp_msg_names[type] ?
           format (s, "%s (%d)", _pfcp_msg_names[type], type) :
...
format_pfcp_msg_header (u8 *s, va_list *args)
{
  ...
  pfcp_msg_type_t type = pfcp->type;

  if (type < ARRAY_LEN (_pfcp_msg_names) && _pfcp_msg_names[type])
...
static const char *source_interface_name[] = {
  [0] = "Access",
  [1] = "Core",
  [2] = "SGi-LAN",
  [3] = "CP-Function",
...
format_pfcp_ie_source_interface (u8 *s, va_list *args)
{
  ...
  return format (s, "%U", format_pfcp_enum, (u64) *v, source_interface_name,
                 ARRAY_LEN (source_interface_name));
}
  ...
  if (!(v->flags & PFCP_F_TEID_CH))
    {
      ...
      if (!(v->flags & (PFCP_F_TEID_V4 | PFCP_F_TEID_V6)))
        {
          upf_debug ("PFCP: F-TEID without v4/v6 address: %02x.", v->flags);
          return -1;
        }
...
static const char *destination_interface_name[] = {
...
format_pfcp_ie_destination_interface (u8 *s, va_list *args)
{
  ...
  return format (s, "%U", format_pfcp_enum, (u64) *v,
                 destination_interface_name,
                 ARRAY_LEN (destination_interface_name));
}
...
decode_pfcp_ie_f_seid (u8 *data, u16 length, void *p)
{
  ...
  if (v->flags == 0)
    {
      upf_debug ("PFCP: F-SEID with unsupported flags: %02x.", v->flags);
      return -1;
    }
...
void
copy_pfcp_ie_node_id (pfcp_ie_node_id_t *dst, pfcp_ie_node_id_t *src)
{
  *dst = *src;
  if (src->type == PFCP_NID_FQDN)
    dst->fqdn = vec_dup (src->fqdn);
}
...
encode_pfcp_tbcd_char (u8 in)
{
  ...
  switch (in)
    ...
    case 'c':
      return 0xe;
    case '?':
      // this is corner case, but we keep it in sync with decode_pfcp_tbcd
      return 0xf;
    }
...
uword
encode_pfcp_tbcd (u8 *in, uword n_in, u8 *out, uword n_out)
{
  u8 *cur = out, *end = out + n_out;
  uword i = 0;
  for (; i < n_in - 1 && cur != end; i += 2)
    {
      u8 v1 = encode_pfcp_tbcd_char (in[i]);
      u8 v2 = encode_pfcp_tbcd_char (in[i + 1]);
      if (v1 == (u8) ~0 || v2 == (u8) ~0)
        return ~0;
  ...
  // zero remaining for consistency
  clib_memset_u8 (cur, 0, n_out - len);
...
format_pfcp_ie_user_id (u8 *s0, va_list *args)
{
  ...
  if (v->imei_len > 0)
    s = format (s, "IMEI:%U,", format_pfcp_tbcd, v->imei, v->imei_len);
  if (v->msisdn_len > 0)
...
static struct pfcp_group_ie_def pfcp_pdi_group[] =
  {
    ...
    [PDI_TRAFFIC_ENDPOINT_ID] = {
      ...
      .offset = offsetof(pfcp_ie_pdi_t, traffic_endpoint_id)
    }
  };
...
static struct pfcp_group_ie_def pfcp_session_report_request_group[] =
  {
    ...
    [SESSION_REPORT_REQUEST_TP_ERROR_MESSAGE] = {
      .type = PFCP_IE_TP_ERROR_MESSAGE,
      .vendor = PFCP_VENDOR_TRAVELPING,
      .offset = offsetof(pfcp_msg_session_report_request_t, tp_error_message)
    },
  };
  ...
  if (def->size != 0)
  ...
  else
    {
      if ((r = def->decode (ie, length, p)) == 0)
        upf_debug ("PFCP: %s: %U.", def->name, def->format, p);
      else
        upf_debug ("FAILED to decode %s", def->name);
      ...
      for (int i = 0; missing; i++, missing >>= 1)
        {
          ...
          upf_debug ("Missing IE Type: %s, %u", ie_def->name,
           ...
           u8 *v, u8 **vec)
{
  ...
  if (def->size != 0)
    ...
    {
      upf_debug ("PFCP: %s: %U.", def->name, def->format, v);
      r = def->encode (v, vec);
    }
...
int
pfcp_encode_msg (pfcp_decoded_msg_t *dmsg, u8 **result_vec)
{
  ...
  int r = encode_group (&msg_specs[dmsg->type], &dmsg->grp, &vec);
  if (r)
    {
      vec_free (vec);
      return r;
...
static format_function_t format_group;
