...
var _ = ginkgo.Describe("UPG Binary API", func() {
	...
	ginkgo.Context("for upf tdf ul table", func() {
		...
		getMappings := func(isIPv6 bool) *upf.UpfTdfUlTableReply {
			...
			gomega.Expect(
				f.VPP.ApiChannel.SendRequest(req).ReceiveReply(reply),
			).To(gomega.Succeed(), "upf_tdf_ul_table")
			return reply
		}
		...
		ginkgo.It("sets and retrieves table mappings", func() {
			...
			gomega.Expect(mappings.MappingsLen).To(gomega.Equal(uint8(0)), "upf_tdf_ul_table")
			gomega.Expect(mappings.Mappings).To(gomega.Equal([]uint32{}), "upf_tdf_ul_table")

			addMapping(true, 0, 1001)
			addMapping(true, 1001, 0)

			mappings = getMappings(true)
			gomega.Expect(mappings.MappingsLen).To(gomega.Equal(uint8(4)), "upf_tdf_ul_table")
			gomega.Expect(mappings.Mappings).To(gomega.Equal([]uint32{0, 1001, 1001, 0}), "upf_tdf_ul_table")

			// there is a mapping for ipv4 initially so we expect it
			mappings = getMappings(false)
			gomega.Expect(mappings.MappingsLen).To(gomega.Equal(uint8(2)), "upf_tdf_ul_table")
			gomega.Expect(mappings.Mappings).To(gomega.Equal([]uint32{100, 1001}), "upf_tdf_ul_table")

			addMapping(false, 1001, 0)

			mappings = getMappings(false)
			gomega.Expect(mappings.MappingsLen).To(gomega.Equal(uint8(4)), "upf_tdf_ul_table")
			gomega.Expect(mappings.Mappings).To(gomega.Equal([]uint32{100, 1001, 1001, 0}), "upf_tdf_ul_table")
		})
	})
