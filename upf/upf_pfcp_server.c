...
upf_pfcp_session_up_deletion_report (upf_session_t * sx)
{
  ...
  /*
   * outlives_session=true so that this request isn't dropped
   * from the response queue after the session is deleted,
   * and can be retransmitted if it's lost
   */
  upf_pfcp_server_send_session_request (sx, &dmsg, true);
  pfcp_free_dmsg_contents (&dmsg);
}
