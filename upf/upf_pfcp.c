...
pfcp_release_association (upf_node_assoc_t * n)
{
  ...
  upf_pfcp_server_stop_and_free_heartbeat_timer (n);
...
pfcp_disable_session (upf_session_t * sx)
{
  ...
  {
    upf_pfcp_session_timer_stop
      (sx, URR_MEASUREMENT_PERIOD_TIMER, &urr->measurement_period);
    upf_pfcp_session_timer_stop
      (sx, URR_TIME_THRESHOLD_TIMER, &urr->time_threshold);
    upf_pfcp_session_timer_stop (sx, URR_TIME_QUOTA_TIMER, &urr->time_quota);
    upf_pfcp_session_timer_stop
      (sx, URR_QUOTA_VALIDITY_TIME_TIMER, &urr->quota_validity_time);
    upf_pfcp_session_timer_stop (sx, URR_TRAFFIC_TIMER, &urr->traffic_timer);
  }
  upf_pfcp_session_timer_stop_and_free
    (sx, URR_UP_INACTIVITY_TIMER, &active->inactivity_timer);
...
pfcp_update_apply (upf_session_t * sx)
{
  ...
  vec_foreach (urr, active->urr)
  {
	...
	if (urr->update_flags & PFCP_URR_UPDATE_TIME_QUOTA)
	  {
	    ...
	    upf_pfcp_session_timer_update (sx, URR_TIME_QUOTA_TIMER,
					   &urr->time_quota,
					   !!(urr->triggers &
	...
	upf_pfcp_session_timer_update (sx, URR_QUOTA_VALIDITY_TIME_TIMER,
	...
	if (!new_urr)
	  {
	    ...
	    upf_pfcp_session_timer_stop
	      (sx, URR_TIME_THRESHOLD_TIMER, &urr->time_threshold);
	    upf_pfcp_session_timer_stop
	      (sx, URR_TIME_QUOTA_TIMER, &urr->time_quota);
	    upf_pfcp_session_timer_stop
	      (sx, URR_QUOTA_VALIDITY_TIME_TIMER, &urr->quota_validity_time);
	    upf_pfcp_session_timer_stop
