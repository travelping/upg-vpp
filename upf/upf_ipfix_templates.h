...
#define IPFIX_VALUE_U8_COND(v, n, c) to_b->data[offset++] = (c) ? (v) : 0
...
#define IPFIX_FIELD_SOURCE_IPV4_ADDRESS(F)                                    \
  F (sourceIPv4Address, 4, IPFIX_VALUE_MEMCPY_DIRECT,                         \
     &f->key.ip[FT_ORIGIN ^ f->is_reverse ^ direction].ip4,                   \
     sizeof (ip4_address_t), 1)
#define IPFIX_FIELD_SOURCE_IPV6_ADDRESS(F)                                    \
  F (sourceIPv6Address, 16, IPFIX_VALUE_MEMCPY_DIRECT,                        \
     &f->key.ip[FT_ORIGIN ^ f->is_reverse ^ direction].ip6,                   \
     sizeof (ip6_address_t), 1)
#define IPFIX_FIELD_DESTINATION_IPV4_ADDRESS(F)                               \
  F (destinationIPv4Address, 4, IPFIX_VALUE_MEMCPY_DIRECT,                    \
     &f->key.ip[FT_REVERSE ^ f->is_reverse ^ direction].ip4,                  \
     sizeof (ip4_address_t), 1)
#define IPFIX_FIELD_DESTINATION_IPV6_ADDRESS(F)                               \
  F (destinationIPv6Address, 16, IPFIX_VALUE_MEMCPY_DIRECT,                   \
     &f->key.ip[FT_REVERSE ^ f->is_reverse ^ direction].ip6,                  \
     sizeof (ip6_address_t), 1)
#define IPFIX_FIELD_PROTOCOL_IDENTIFIER(F)                                    \
  F (protocolIdentifier, 1, IPFIX_VALUE_DIRECT, f->key.proto, 1, 1)
#define IPFIX_FIELD_MOBILE_IMSI(F)                                            \
  F (455, 65535, IPFIX_VALUE_MOBILE_IMSI, sx->user_id.imsi,                   \
     sx->user_id.imsi_len, 1)
#define IPFIX_FIELD_INITIATOR_PACKETS(F)                                      \
  F (initiatorPackets, 8, IPFIX_VALUE_DELTA_U64,                              \
     flow_stats (f, FT_ORIGIN).pkts_unreported, sizeof (u64), 1)
#define IPFIX_FIELD_RESPONDER_PACKETS(F)                                      \
  F (responderPackets, 8, IPFIX_VALUE_DELTA_U64,                              \
     flow_stats (f, FT_REVERSE).pkts_unreported, sizeof (u64), 1)
#define IPFIX_FIELD_INITIATOR_OCTETS(F)                                       \
  F (initiatorOctets, 8, IPFIX_VALUE_DELTA_U64,                               \
     flow_stats (f, FT_ORIGIN).l4_bytes_unreported, sizeof (u64), 1)
#define IPFIX_FIELD_RESPONDER_OCTETS(F)                                       \
  F (responderOctets, 8, IPFIX_VALUE_DELTA_U64,                               \
     flow_stats (f, FT_REVERSE).l4_bytes_unreported, sizeof (u64), 1)
#define IPFIX_FIELD_PACKET_DELTA_COUNT(F)                                     \
  F (packetDeltaCount, 8, IPFIX_VALUE_DELTA_U64,                              \
     flow_stats (f, direction).pkts_unreported, sizeof (u64), 1)
#define IPFIX_FIELD_OCTET_DELTA_COUNT(F)                                      \
  F (octetDeltaCount, 8, IPFIX_VALUE_DELTA_U64,                               \
     flow_stats (f, direction).bytes_unreported, sizeof (u64), 1)
#define IPFIX_FIELD_PACKET_TOTAL_COUNT(F)                                     \
  F (packetTotalCount, 8, IPFIX_VALUE_U64, flow_stats (f, direction).pkts,    \
     sizeof (u64), 1)
#define IPFIX_FIELD_OCTET_TOTAL_COUNT(F)                                      \
  F (octetTotalCount, 8, IPFIX_VALUE_U64, flow_stats (f, direction).bytes,    \
     sizeof (u64), 1)
#define IPFIX_FIELD_FLOW_START_MILLISECONDS(F)                                \
  F (flowStartMilliseconds, 8, IPFIX_VALUE_DATETIME_MILLISECONDS,             \
     f->flow_start_time, sizeof (u64), 1)
#define IPFIX_FIELD_FLOW_END_MILLISECONDS(F)                                  \
  F (flowEndMilliseconds, 8, IPFIX_VALUE_DATETIME_MILLISECONDS,               \
     f->flow_end_time, sizeof (u64), 1)
#define IPFIX_FIELD_FLOW_DIRECTION(F)                                         \
  F (flowDirection, 1, IPFIX_VALUE_DIRECT, direction == FT_ORIGIN ? 1 : 0, 1, \
     1)
#define IPFIX_FIELD_SOURCE_TRANSPORT_PORT(F)                                  \
  F (sourceTransportPort, 2, IPFIX_VALUE_MEMCPY_DIRECT,                       \
     &f->key.port[FT_ORIGIN ^ f->is_reverse ^ direction], 2, 1)
#define IPFIX_FIELD_DESTINATION_TRANSPORT_PORT(F)                             \
  F (destinationTransportPort, 2, IPFIX_VALUE_MEMCPY_DIRECT,                  \
     &f->key.port[FT_REVERSE ^ f->is_reverse ^ direction], 2, 1)
#define IPFIX_FIELD_POST_NAT_IPV4_ADDRESS(F)                                  \
  F (postNATSourceIPv4Address, 4, IPFIX_VALUE_MEMCPY_DIRECT,                  \
     &sx->nat_addr->ext_addr, sizeof (ip4_address_t), 1)
#define IPFIX_FIELD_POST_NAPT_SOURCE_TRANSPORT_PORT(F)                        \
  F (postNAPTSourceTransportPort, 2, IPFIX_VALUE_U16_COND, f->nat_sport,      \
     sizeof (u16), sx->nat_addr)
#define IPFIX_FIELD_POST_NAT_SOURCE_IPV4_ADDRESS(F)                           \
  F (postNATSourceIPv4Address, 4, IPFIX_VALUE_MEMCPY_DIRECT_COND,             \
     &sx->nat_addr->ext_addr, sizeof (ip4_address_t), sx->nat_addr)
#define IPFIX_FIELD_INGRESS_VRF_ID(F)                                         \
  F (ingressVRFID, 4, IPFIX_VALUE_U32, info->ingress_vrf_id, sizeof (u32), 1)
#define IPFIX_FIELD_EGRESS_VRF_ID(F)                                          \
  F (egressVRFID, 4, IPFIX_VALUE_U32, info->egress_vrf_id, sizeof (u32), 1)
#define IPFIX_FIELD_VRF_NAME(F)                                               \
  F (VRFname, 65535, IPFIX_VALUE_STRING, info->vrf_name,                      \
     vec_len (info->vrf_name), 1)
#define IPFIX_FIELD_INTERFACE_NAME(F)                                         \
  F (interfaceName, 65535, IPFIX_VALUE_STRING, info->interface_name,          \
     vec_len (info->interface_name), 1)
#define IPFIX_FIELD_OBSERVATION_DOMAIN_NAME(F)                                \
  F (observationDomainName, 65535, IPFIX_VALUE_STRING,                        \
     info->observation_domain_name, vec_len (info->observation_domain_name),  \
     1)
#define IPFIX_FIELD_OBSERVATION_POINT_ID(F)                                   \
  F (observationPointId, 8, IPFIX_VALUE_U64, info->observation_point_id,      \
     sizeof (u64), 1)

#define UPF_NAT_EVENT_NAT44_SESSION_CREATE 4
#define UPF_NAT_EVENT_NAT44_SESSION_DELETE 5
#define IPFIX_FIELD_NAT_EVENT(F)                                              \
  F (natEvent, 1, IPFIX_VALUE_U8_COND,                                        \
     !f->exported ? UPF_NAT_EVENT_NAT44_SESSION_CREATE :                      \
     last         ? UPF_NAT_EVENT_NAT44_SESSION_DELETE :                      \
                    0,                                                                \
     sizeof (u8), 1)
...
#define IPFIX_VALUE(fieldName, specLen, valCopy, v, n, c) valCopy (v, n, c);

#define IPFIX_COUNT(fieldName, specLen, valCopy, v, n, c) 1 +

#define IPFIX_TEMPLATE_COUNT(T1, T2) (T1 (IPFIX_COUNT) T2 (IPFIX_COUNT) 0)
