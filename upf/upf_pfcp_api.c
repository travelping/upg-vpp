...
static void
expunge_conflicting_session (upf_session_t * sx_old,
			     ...
			     pfcp_ue_ip_address_t * ue_addr)
{
  ...
  upf_pfcp_server_deferred_free_msgs_by_sidx (sx_old - gtm->sessions);
  upf_pfcp_session_up_deletion_report (sx_old);

  if ((r = pfcp_disable_session (sx_old)) != 0)
			 ...
			 pfcp_ue_ip_address_t * ue_addr)
{
  ...
  if (ue_addr->flags & IE_UE_IP_ADDRESS_V4)
	...
	{
	  expunge_conflicting_session (gtm->sessions + dpo->dpoi_index, sx,
				       ue_addr);
	  r = -1;
	}
    }
