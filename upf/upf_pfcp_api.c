  ...
  if ((pdi->f_teid.flags & F_TEID_CH))
    {
      ...
      if ((!is_zero_ip4_address (&res->ip4)) && (pdi->f_teid.flags & F_TEID_V4))
        {
          process_pdr->pdi.teid.ip4 = res->ip4;
          process_pdr->pdi.teid.flags |= F_TEID_V4;
        }

      if ((!is_zero_ip6_address (&res->ip6)) && (pdi->f_teid.flags & F_TEID_V6))
        {
		   ...
		   pfcp_failed_rule_id_t * failed_rule_id)
{
  ...
  vec_foreach (pdr, create_pdr)
  {
    ...
    if (ISSET_BIT (pdr->pdi.grp.fields, PDI_F_TEID))
      {
        if (handle_f_teid(sx, gtm, &pdr->pdi, create, created_pdr_vec, res, grp, 1) != 0)
          {
            r = -1;
		   ...
		   pfcp_failed_rule_id_t * failed_rule_id)
{
  ...
  vec_foreach (pdr, update_pdr)
  {
    ...
    if (ISSET_BIT (pdr->pdi.grp.fields, PDI_F_TEID))
      {
        if (handle_f_teid(sx, gtm, &pdr->pdi, update, NULL, res, grp, 0) != 0)
          {
            r = -1;
            break;
          }
